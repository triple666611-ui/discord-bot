...
class ShopCog(commands.Cog):
    ...
    def _build_catalog_embed(self, user_id: int) -> discord.Embed:
        ...
        embed.set_footer(
            text=(
                "Покупка: /shop action:buy item:<ключ> | "
                "Активация: /shop action:use item:<ключ> | "
                "Отключение: /shop action:deactivate item:<ключ>"
            )
        )
        return embed
    ...
    def _build_inventory_embed(self, user_id: int) -> discord.Embed:
        ...
        profile = self.profile_service.get_profile(user_id)
        active_cosmetics = self.shop_service.get_active_cosmetics(user_id)
    ...
    @app_commands.choices(
        action=[
            ...
            app_commands.Choice(name="деактивировать", value="deactivate"),
