...
class Games(commands.Cog):
    ...
    async def daily_cmd(self, interaction: Interaction) -> None:
        ...
        embed = discord.Embed(
            ...
            description=f"Ты получил **{Config.DAILY_REWARD}** 🪙\nНовый баланс: **{new_balance}** 🪙",
    ...
    async def coin_cmd(self, interaction: Interaction, side: app_commands.Choice[str], bet: int) -> None:
        ...
        if win:
            ...
            embed = self._build_game_embed(
                ...
                game_name="Монетка",
                bet=bet,
                result_lines=[f"Ты выбрал: **{side.value}**", f"Выпало: **{result}**", f"Выигрыш: **+{bet}** 🪙"],
                balance_after=new_balance,
            )
            result_text = f"Победа, выпало {result}"
        else:
            new_balance = self.profile_service.add_balance(interaction.user.id, -bet)
            embed = self._build_game_embed(
    ...
    async def dice_cmd(self, interaction: Interaction, bet: int) -> None:
        ...
        if user_roll > bot_roll:
            ...
            embed = self._build_game_embed(
                ...
                game_name="Кости",
                bet=bet,
                result_lines=[f"Твой бросок: **{user_roll}**", f"Бросок бота: **{bot_roll}**", f"Выигрыш: **+{bet}** 🪙"],
                balance_after=new_balance,
            )
            result_text = f"Победа: игрок {user_roll}, бот {bot_roll}"
